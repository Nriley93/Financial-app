...
public class FinancialsPVView extends FrameView {
    ...
    private void jbtnCalcActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnCalcActionPerformed
        // Remember do all with try catch with final version
        double a, r;
        int t;
        ...
        try {
            ...
            statusMessageLabel.setText("Amount value not allowed. ");
        ...
        try {
            ...
            statusMessageLabel.setText("Rate value not allowed. ");
        ...
        try {
            ...
            statusMessageLabel.setText("Term value not allowed. ");
        ...
        if(jradAnnuity.isSelected()){
            ...
            statusMessageLabel.setText("Unknown or missing operation. ");
            return;
        } if(!f.getErrorMsg().isEmpty()){
            statusMessageLabel.setText(f.getErrorMsg());
            return;
        } // End of objects
    ...
    private void jradAnnuityItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jradAnnuityItemStateChanged
        if(jradAnnuity.isSelected()){
            jLabel1.setText(Annuity.AMTDESC + ":");
    ...
    private void jbtnSchedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnSchedActionPerformed
        ...
        String[][] t;
        if(f instanceof PV) {
            cols = new String[] {"Month", PV.DISCOUNTDESC, PV.RESULTDESC};
            t = new String [f.getTerm()+1][3];
            ...
            sched.setDefaultRenderer(Object.class, rightRender);
            // End alignment

            // Start Schedule PV
            for (int i = 0; i < f.getTerm()+1; i++){
                sched.setValueAt(i, i, 0);
                sched.setValueAt(
                        curr.format(f.getBegBal(i+1)), i, 1);
                sched.setValueAt(
                        curr.format(f.getEndBal(i+1)), i, 2);
                sched.getColumnModel().getColumn(0).setCellRenderer(centerRender);
            } // End of for loop           
        } else {
        
            if(f instanceof Annuity) {
                cols = new String[] {"Month", Annuity.BEGBALDESC, Annuity.PRINDESC,
                                    Annuity.INTERESTDESC, Annuity.ENDBALDESC };
                t = new String [f.getTerm()][5];
            } else if (f instanceof Loan) {
                cols = new String[] {"Month", Loan.BEGBALDESC, Loan.PAYMENTDESC,
                    Loan.INTERESTDESC, Loan.ENDBALDESC, Loan.PRINDESC};
            ...
            for (int i = 0; i < f.getTerm(); i++){
                ...
                sched.setValueAt(
                        curr.format(f.getBegBal(i+1)),i,1);
                sched.setValueAt(
                        curr.format(f.getPrincipal(i+1)),i,2);
                sched.setValueAt(
                        curr.format(f.getInterest(i+1)),i,3);
                sched.setValueAt(
                        curr.format(f.getEndBal(i+1)),i,4);
                if(f instanceof Loan) {
                sched.setValueAt(
                        curr.format(((Loan)f).getPrinPaid(i+1)),i, 5);
                }
                sched.getColumnModel().getColumn(0).setCellRenderer(centerRender);
            } // End of for loop
        } // End of Big else
